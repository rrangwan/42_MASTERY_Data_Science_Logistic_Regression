...
def evaluate(train_path: str,
    ...
    # Align the two Series by their index and keep only matching values
    # y_pred_aligned, y_true_aligned = y_pred.align(y_true, join='inner')  # 'inner' means keep only common indices

    # print("Wrong predictions:", np.sum(y_true_aligned != y_pred_aligned))
    # print("Accuracy:", np.round(accuracy_score(y_true_aligned, y_pred_aligned), 4))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--truth_path', type=str,
                        default="datasets/dataset_truth.csv",
